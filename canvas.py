...
class CanvasConnect:
    def __init__(self, COURSE, YEAR, ASSIGNMENT, API_KEY = "canvas_api_key.txt", CANVAS = "https://canvas.vu.nl/"):
        ...
        self.course = next(course for course in self.canvas.get_courses() if getattr(course, "course_code", None) == COURSE and getattr(getattr(course, "start_at_date", None), "year", getattr(getattr(course, "created_at_date", None), "year", None)) == int(YEAR))
